...
class Stock:
    ...
    def get_yest_close(self, ContextInfo) -> float:
        return self.get_yest_data(ContextInfo, ["close"])["close"]

    def get_price(self, ContextInfo) -> float:
    ...
    def is_up(self, ContextInfo) -> bool:
    ...
    def is_down(self, ContextInfo) -> bool:
        current_price = self.get_price(ContextInfo)
        yest_close = self.get_yest_close(ContextInfo)
        down_price = yest_close * self.get_down_rate()
        return abs(current_price - down_price) < 0.01

    def get_avg_amp(self, ContextInfo, days: int) -> float:
    ...
    def get_avg_price(self, ContextInfo, days: int) -> float:
