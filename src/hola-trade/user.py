...
class User:
    ...
    def get_share(self, money, price) -> int:
        return int((money // (price * 100)) * 100)

    def order_by_value(self, ContextInfo, code: str, cash: float, price=0):
        if cash:
            if price:
                self.log.log_info(ContextInfo, f" order_by_cash: code:{code} at price:{price} with cash:{cash}")
                self.ctx.order_value(code, cash, 'FIX', price, ContextInfo, self.id)
            else:
                self.log.log_info(ContextInfo, f"order_by_cash: code:{code} with cash:{cash} at latest price")
                self.ctx.order_value(code, cash, ContextInfo, self.id)

    def order_by_shares(self, ContextInfo, code: str, share: int, price=0):
        if share:
            if price:
                self.log.log_info(ContextInfo, f"order_by_shares: code:{code} at price:{price} with share:{share}")
                self.ctx.order_shares(code, share, 'FIX', price, ContextInfo, self.id)
            else:
                self.log.log_info(ContextInfo, f"order_by_shares: code:{code} with share:{share} at latest price")
